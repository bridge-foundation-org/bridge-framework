...
use protocol::{parse_command, Command, Response};
...
fn exec(cmd: Command, state: SharedState) -> Response {
    ...
    match cmd {
        ...
        Command::CompileFile { path } => match std::fs::read_to_string(&path) {
        ...
        // ── KV store ──────────────────────────────────────────────────────
        Command::DbPut { ns, key, value } => {
            g.store.put(&ns, &key, value);
            Response::Ok("stored".into())
        }
        ...
        Command::DbFlush { ns } => {
            g.store.flush_ns(&ns);
            Response::Ok("flushed".into())
        }

        // ── Postgres (drop lock before Docker I/O) ────────────────────────
        Command::PgCreate { name } => {
            drop(g);
            pg_result(sqldb::create(&name))
        }
        Command::PgStatus => {
            drop(g);
            pg_result(sqldb::status())
        }
        Command::PgMigrate { sql } => {
            drop(g);
            pg_result(sqldb::migrate(&sql))
        }
        Command::PgDestroy { name } => {
            drop(g);
            pg_result(sqldb::destroy(&name))
        }
        ...
        Command::AuthClear => {
            g.auth_token = None;
            Response::Ok("auth token cleared".into())
        }
        ...
        Command::TraceClear => {
            g.traces.clear();
            Response::Ok("traces cleared".into())
        }
        ...
        Command::MetricsGet { name } => {
            ...
            Response::Data(format!(
                r#"{{"endpoint":"{name}","requests":{count},"errors":{errs}}}"#
            ))
        }
        Command::MetricsClear => {
            g.metrics = Default::default();
            Response::Ok("metrics cleared".into())
        }
        ...
        Command::PubSubSubscribe { topic, subscriber } => {
            g.pubsub.subscribe(
                &topic,
                &subscriber,
                crate::pubsub::SubscriptionConfig::default(),
            );
            Response::Ok(format!("subscribed {subscriber} to {topic}"))
        }
        Command::PubSubPull { topic, subscriber } => match g.pubsub.pull(&topic, &subscriber) {
        ...
        Command::SecretGet { name } => match g.secrets.get(&name) {
        ...
        Command::StreamOpen { path } => match g.streams.open(&path) {
            Some(id) => {
                g.streams.set_open(&id);
                Response::Ok(format!("stream opened id={id}"))
            }
...
mod tests {
    ...
    #[test]
    fn ping() {
        assert_eq!(process("PING", state()).trim(), "PONG");
    }
    #[test]
    fn mode_get() {
        assert!(process("MODE GET", state()).starts_with("MODE "));
    }
    #[test]
    fn mode_set() {
        assert!(process("MODE SET lite", state()).starts_with("OK "));
    }
    #[test]
    fn version() {
        assert!(process("VERSION", state()).starts_with("DATA "));
    }
    #[test]
    fn health() {
        assert!(process("HEALTH", state()).starts_with("DATA "));
    }
    ...
    fn services_list_after_compile() {
        ...
        assert!(
            r.contains("hello"),
            "expected service name 'hello', got: {r}"
        );
    }
    ...
    fn db_put_get_del() {
        ...
        assert!(
            r.contains("hello%20world") || r.contains("hello world"),
            "got: {r}"
        );
