...
impl Entry {
    fn str(v: String) -> Self {
        Entry {
            val: StoreVal::String(v),
            expires_at: None,
        }
    }
    ...
    fn list() -> Self {
        Entry {
            val: StoreVal::List(vec![]),
            expires_at: None,
        }
    }
    fn hash() -> Self {
        Entry {
            val: StoreVal::Hash(HashMap::new()),
            expires_at: None,
        }
    }
...
impl Store {
    ...
    fn incr_by(&mut self, key: &str, by: i64) -> Resp {
        ...
        let next = cur + by;
        self.data
            .insert(key.to_string(), Entry::str(next.to_string()));
        Resp::int(next)
    }
        ...
        match self.live_mut(key) {
            ...
            Some(e) => match &mut e.val {
                StoreVal::Hash(h) => {
                    let removed = fields
                        .iter()
                        .filter(|f| h.remove(f.as_str()).is_some())
                        .count();
...
fn dispatch_inner(args: &[String], store: &mut Store) -> Result<Resp, Resp> {
    if args.is_empty() {
        return Ok(Resp::err("ERR no command"));
    }
    ...
    Ok(match cmd.as_str() {
        ...
        "GET" => {
            if args.len() < 2 {
                return Err(Resp::err("ERR wrong number of arguments for 'get' command"));
            }
            store.get(&args[1])
        }
        "SETNX" => {
        ...
        "SET" => {
            ...
            while i < args.len() {
                match args[i].to_ascii_uppercase().as_str() {
                    "EX" => {
                        i += 1;
                        ttl = Some(Duration::from_secs(parse_u64(args.get(i))?));
                    }
                    "PX" => {
                        i += 1;
                        ttl = Some(Duration::from_millis(parse_u64(args.get(i))?));
                    }
        ...
        "EXPIRE" => {
            require(args, 3)?;
            store.expire(&args[1], parse_u64(args.get(2))?)
        }
        "TTL" => {
            require(args, 2)?;
            store.ttl(&args[1])
        ...
        "INCRBY" => {
            require(args, 3)?;
            store.incr_by(&args[1], parse_i64(args.get(2))?)
        }
        "DECRBY" => {
            require(args, 3)?;
            store.incr_by(&args[1], -parse_i64(args.get(2))?)
        ...
        "LPUSH" => {
            require_min(args, 3)?;
            store.lpush(args[1].clone(), &args[2..])
        }
        "RPUSH" => {
            require_min(args, 3)?;
            store.rpush(args[1].clone(), &args[2..])
        }
        "LRANGE" => {
            require(args, 4)?;
        ...
        "LINDEX" => {
            require(args, 3)?;
            store.lindex(&args[1], parse_i64(args.get(2))?)
        }
        ...
        "HINCRBY" => {
            require(args, 4)?;
            store.hincrby(&args[1], &args[2], parse_i64(args.get(3))?)
        }
...
impl MiniRedis {
    ...
    pub fn start(addr: &str) -> Result<(Self, JoinHandle<()>), String> {
        ...
        let handle = thread::spawn(move || loop {
            if shut2.load(Ordering::Relaxed) {
                break;
            }
...
mod tests {
    ...
    fn mget_mset() {
        ...
        assert_eq!(
            r,
            Resp::Array(vec![Resp::bulk("1"), Resp::bulk("2"), Resp::null()])
        );
    }
    #[test]
    fn incr_decr() {
    ...
    fn hkeys_hvals() {
        ...
        assert_eq!(
            s.hvals("h"),
            Resp::Array(vec![Resp::bulk("1"), Resp::bulk("2")])
        );
    }
